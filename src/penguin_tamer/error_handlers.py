...
def connection_error(error: Exception) -> str:
    ...
    if isinstance(error, _get_openai_exceptions()['RateLimitError']):
        ...
        return t(
            "[dim]Error API: {error}. Check the LLM settings, "
            "there may be an incorrect API_URL[/dim]"
        ).format(error=error)
    elif isinstance(error, _get_openai_exceptions()['OpenAIError']):
        return t(
            "[dim]Please check your API_KEY. See provider docs for obtaining a key. "
            "[link={link}]How to get a key?[/link][/dim]"
