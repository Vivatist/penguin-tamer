...
class OpenRouterClient(AbstractLLMClient):
    ...
    def _extract_rate_limits(self, stream) -> None:
        """Extract rate limit information from OpenRouter API response.
        
        OpenRouter uses both OpenAI-style headers and its own custom headers.
        Tries multiple header formats for maximum compatibility.
        
        Args:
            stream: OpenRouter API response stream object
        """
        try:
            ...
            if headers:
                # OpenAI style headers (preferred)
                ...
                # OpenRouter legacy headers (fallback)
