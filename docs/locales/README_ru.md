...
> **Интеграция искусственного интеллекта в ваш терминал.** Задавайте вопросы ИИ и выполняйте предложенные скрипты и команды прямо из командной строки. Идеально подходит для новичков в администрировании Linux и Windows.
...
- **Отсутствие интерфейса** - Общайтесь с выбранным ИИ в естественной форме: ai как утановить русский шрифт?
...
- **Готова к локализации** - Поддержка нескольких языков (в разработке) [(в разработке)](#содействие)
...
![start program](/docs/img/ru_img1.png)

Через пару секунд нейросеть ответит:

![pgram response1](/docs/img/ru_img2.png)

Не очень вежливо, по желанию это поведение можно изменить в настройках задав свой контент для нейросети. Попробуйте что нибудь посложнее:

![pgram response1](/docs/img/ru_img3.png)

При первом запуске программа использует нейросеть от OpenAi - ChatGPT модель gpt-4o-mini с бесплатным доступом. Это не самый лучший выбор, так как при множестве запросов будет периодически появляться сообщение о превышении квоты запросов, но для тестового запуска сойдет. 

Для полноценной работы рекомендую сменить провайдера в [настройках](#конфигурация). По умолчанию в программу уже добавлено несколько популярну моделей, таких как **DeepSeek**, **Grok 4 Fast**, **Qwen3 Coder**. 

Для того чтобы эти нейросети заработали необходимо получить личный токен и добавить его в натройки конкретной нейросети в [настройках](#установка) программы.
...
- 🌍 **Локализация** - Добавление поддержки новых языков
